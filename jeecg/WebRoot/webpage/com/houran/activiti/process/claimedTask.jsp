...
<script type="text/javascript">
		...
		function getData(data){
			...
			for(var i=0; i<data.rows.length; i++){
				rows.push({
					...
					opt: "[<a href=\"#\" onclick=\"add('办理','oaLeavebillFormController.do?taskCompletePageSelect&jspPage="+data.rows[i].description+"&processInstanceId="+data.rows[i].processInstanceId+"&taskId="+data.rows[i].taskId+"','claimedTask')\">办理</a>]"
				});
			}
		...
		$('#claimedTask').datagrid({
	        ...
	        fitColumns:true,
	        showFooter:true,
	        url:'oaLeavebillFormController.do?claimedTaskDataGrid',  
